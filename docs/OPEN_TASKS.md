...
- Continue P6 multi-agent dispatch so Codex can assign separate tasks to MiMo and Reasonix instead of choosing only one active provider. P6.0/P6.1 Agent Registry/probe and P6.2 fake Reasonix one-shot runner are implemented. Next is controlled real Reasonix one-shot smoke on a disposable repo, then Reasonix session mapping. Do not expose broad UI task creation until real smoke and bounded Review Package behavior are verified.
...
- P6.2 fake Reasonix one-shot runner: added `agent_start_task`, `/api/agent-tasks`, `ReasonixTuiRunner`, fake Reasonix fixture, and Worktree Review Package test. Verified with root/local-daemon builds and 34/34 targeted tests. Real Reasonix task execution is still pending controlled smoke.
## Risks
...
5. Continue P6 with the design in `docs/modules/multi-agent-dispatch.md` and `docs/modules/reasonix-tui-adapter.md`: next slice is real Reasonix one-shot smoke on a disposable repo, then Reasonix session mapping, agent-aware queue/path-conflict scheduling, MiMo adapter migration to generic tools, and GUI shared-session viewing.
