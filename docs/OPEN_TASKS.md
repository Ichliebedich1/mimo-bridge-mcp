...
- Continue P6 multi-agent dispatch so Codex can assign separate tasks to MiMo and Reasonix instead of choosing only one active provider. P6.0/P6.1 Agent Registry/probe, P6.2 Reasonix one-shot runner plus generic low-token get/wait/session mapping, and P6.3 first Admin UI agent selector/badges are implemented. Next is agent-aware queue/path-conflict scheduling, then MiMo adapter migration to generic tools and GUI shared-session viewing.
...
5. Continue P6 with the design in `docs/modules/multi-agent-dispatch.md` and `docs/modules/reasonix-tui-adapter.md`: next slice is agent-aware queue/path-conflict scheduling, then MiMo adapter migration to generic tools and GUI shared-session viewing.
