...
- Continue P6 multi-agent dispatch so Codex can assign separate tasks to MiMo and Reasonix instead of choosing only one active provider. P6.0-P6.9 are implemented through Reasonix TUI run/wait/reply/live/session-folder/token and generic lifecycle slices. Next is deeper MiMo adapter migration behind the generic tools, direct Reasonix GUI shared-session opening if a stable command exists, and final packaging/release validation.
