...
当前 MCP 服务使用 `stdio`，队列、运行任务注册表和 Token 预算都保存在进程内存中。浏览器不能直接调用 `stdio`，也不能另起一个独立 MCP 进程，否则界面和 Codex 会看到不同的运行状态。
...
在 P4 修复前，如果已有写任务运行，界面必须禁用“开始任务”和“发送回复”，而不是依赖当前队列实现。
...
显示 `running`、`queued` 和队列项。P4 修复前固定显示警告：返回 `queued` 不代表任务一定没有提前启动。
...
4. 没有事件推送；第一版只能轮询。
5. P4 队列行为审核未通过，界面必须临时限制同时发起写任务。
...
在 P4 队列行为彻底修复前，UI 继续采用保守限制：

- 只要 `/api/queue.running` 或任务列表中存在 `running` 状态任务，即视为已有写任务运行。
- 此时禁用“开始任务”和“发送回复”。
- 队列页固定显示 queued 不保证未提前启动的风险提示。
