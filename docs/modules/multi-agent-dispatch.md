...
## P6.25 Low-Cost Routing And Model Profiles

Status: implemented locally. The goal is to reduce cost without adding a heavy Codex reasoning step before every delegation.

Implemented behavior:

- Uses task scenarios: `multimodal`, `simple`, `normal`, `complex`, and `high_risk`.
- Stores default scenario selections in persistent `routingProfiles`.
...
- MiMo stores the user-facing selected model in task config, task briefs, and Review Package. Before execution, `mimo-runner` maps Bridge labels to MiMo CLI `provider/model` names: `mimo-v2.5 -> xiaomi/mimo-v2.5`, legacy `mimo-v2.5-flash -> xiaomi/mimo-v2.5`, `mimo-v2.5-pro -> xiaomi/mimo-v2.5-pro`. Already-qualified values containing `/` pass through unchanged.
...
- Multimodal/image tasks are forced to MiMo `mimo-v2.5`.
- Model lists are enforced: MiMo supports `mimo-v2.5` / `mimo-v2.5-pro`; Reasonix supports `deepseek-v4-flash` / `deepseek-v4-pro`. Legacy `mimo-v2.5-flash` is accepted only as an alias for existing tasks/config.
...
Current default policy is rule-based and quiet:
...
Remaining routing refinements:

- MiMo and Reasonix now share the same routing policy and `TaskScopePolicy`.
...
- MiMo CLI compatibility depends on the installed provider/model list. On the current machine, `mimo models` lists `xiaomi/mimo-v2.5` and `xiaomi/mimo-v2.5-pro`; passing raw legacy `mimo-v2.5-flash` caused `ProviderModelNotFoundError`, so runners must map Bridge labels and aliases to provider/model IDs before invoking the CLI.
