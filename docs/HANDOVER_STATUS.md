...
Next engineering action: restart/deploy the local daemon after P6.6 so the live viewer can load the new Reasonix session parser at runtime; then continue with generic MiMo adapter migration, backend-safe GUI/session opening, or Reasonix token/cost extraction if stable fields exist. For further scripted MiMo/Reasonix collaboration, use ``scripts/mimo-bridge-client.mjs``/``.ps1`` where available, run ``recover`` after any interrupted wait, keep using Review Package first, and escalate to focused diff only when risk flags, ``use_worktree=false``, or review notes require it.
