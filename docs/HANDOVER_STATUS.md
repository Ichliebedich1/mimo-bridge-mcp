...
MiMo Bridge MCP after P0-P6.6 implementation, TokenBudget real-event hookup, documentation cleanup, Windows 10/11 x64 packaging preparation, and the first Reasonix TUI multi-agent scheduling/reply/live-session slices.
...
- P6.7 safe local-open first slice is implemented locally: Admin UI task detail can ask the localhost daemon to open a task folder or Reasonix session folder. The browser sends only `action`, not a path; the daemon resolves and validates stored task paths before opening, and does not return raw local paths.
...
- P6.7 safe local-open first slice is complete in code and focused tests: `apps/local-daemon/src/task-open-actions.ts` resolves task folders from active Worktree or original allowed workspace, resolves Reasonix session folders under configured `REASONIX_HOME`, and rejects arbitrary/out-of-bound paths. Admin UI calls `POST /api/tasks/:id/open` through fixed actions. Verified with root build, local-daemon build, admin-ui build, and `node --test tests\task-open-actions.test.mjs tests\admin-api.test.mjs` passing 31/31.
...
4. Restart/deploy the current local daemon after this commit so it loads the new `agent_session_path` persistence plus `agent_get_task` / `agent_wait_task` MCP tools and `/api/agent-tasks/:id` REST routes. Current port 3210 daemon is healthy and already has pending review recovery loaded, but it may not yet expose this newest P6.2 session slice until restarted.
5. Continue P6 multi-agent dispatch for MiMo plus Reasonix. P6 target is Reasonix reaching MiMo-level Bridge capability, with Reasonix TUI as the first execution path and Reasonix GUI initially limited to shared-session viewing/manual companion behavior. P6 must reuse P5.5 `TaskScopePolicy`. Next engineering slices are gradual MiMo adapter migration to the generic `agent_*` layer and direct Reasonix GUI/session opening if stable support exists.
6. Continue safe session opening if needed: task-folder and Reasonix session-folder opening are implemented; direct Reasonix GUI-to-specific-session opening remains future work and should only be added if Reasonix exposes a stable command/deep link.
...
Next engineering action: restart/deploy the local daemon after P6.8 so live parsing, safe local-open actions, and Reasonix token extraction are available at runtime; then continue with generic MiMo adapter migration or direct Reasonix GUI/session opening if a stable command exists. For further scripted MiMo/Reasonix collaboration, use ``scripts/mimo-bridge-client.mjs``/``.ps1`` where available, run ``recover`` after any interrupted wait, keep using Review Package first, and escalate to focused diff only when risk flags, ``use_worktree=false``, or review notes require it.
