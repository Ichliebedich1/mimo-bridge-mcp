...
- P6.2 runner/control slice is implemented locally: `agent_start_task`, `agent_get_task`, `agent_wait_task`, `/api/agent-tasks`, `/api/agent-tasks/:id`, `/api/agent-tasks/:id/wait`, and `ReasonixTuiRunner` can run and review one-shot Reasonix-style tasks through the existing Worktree, queue, dynamic scope, low-token wait, and Review Package flow. This is verified with fake Reasonix and a controlled real Reasonix smoke.
...
- P6.2 one-shot runner and generic low-token task control are complete in code and tests: `agent_start_task(agent_id="reasonix-tui")` creates a Worktree, runs a Reasonix-compatible command, writes bounded visible output to Bridge JSONL logs, updates status to `review`, and generates a Review Package with changed files. `agent_get_task` and `agent_wait_task` now expose the same bounded review/wait behavior for any agent with optional `agent_id` mismatch protection. Controlled real smoke task `task_f8b579217015` succeeded with `max_steps=20`; `max_steps=5` was too low and caused a false failure. The smoke task's temporary Worktree was discarded, so it is no longer an active Worktree risk. Verified with root/local-daemon builds and `node --test tests/agent-get-wait-task.test.mjs tests/agent-start-task.test.mjs tests/admin-api.test.mjs tests/stdio-protocol.test.mjs` passing 36/36.
...
4. Restart/deploy the current local daemon after this commit so it loads the new `agent_get_task` / `agent_wait_task` MCP tools and `/api/agent-tasks/:id` REST routes. Current port 3210 daemon is healthy and already has pending review recovery loaded, but it may not yet expose this newest P6.2 control slice until restarted.
5. Continue P6 multi-agent dispatch for MiMo plus Reasonix. P6 target is Reasonix reaching MiMo-level Bridge capability, with Reasonix TUI as the first execution path and Reasonix GUI initially limited to shared-session viewing/manual companion behavior. P6 must reuse P5.5 `TaskScopePolicy`. Next engineering slice is Reasonix session mapping (`agent_session_path`) so TUI runs can be associated with Reasonix GUI/shared session records.
...
Next engineering action: restart/deploy the local daemon so `agent_get_task`, `agent_wait_task`, `/api/agent-tasks/:id`, and `/api/agent-tasks/:id/wait` are available at runtime; then implement Reasonix session mapping. For further scripted MiMo/Reasonix collaboration, use ``scripts/mimo-bridge-client.mjs``/``.ps1`` where available, run ``recover`` after any interrupted wait, keep using Review Package first, and escalate to focused diff only when risk flags, ``use_worktree=false``, or review notes require it.
