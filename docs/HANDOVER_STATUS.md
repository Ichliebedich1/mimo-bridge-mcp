...
Next engineering action: restart/deploy the local daemon after P6.7 so live parsing and safe local-open actions are available at runtime; then continue with generic MiMo adapter migration, direct Reasonix GUI/session opening if a stable command exists, or Reasonix token/cost extraction if stable fields exist. For further scripted MiMo/Reasonix collaboration, use ``scripts/mimo-bridge-client.mjs``/``.ps1`` where available, run ``recover`` after any interrupted wait, keep using Review Package first, and escalate to focused diff only when risk flags, ``use_worktree=false``, or review notes require it.
