...
Next engineering action: restart/deploy the local daemon so `agent_session_path`, `agent_get_task`, `agent_wait_task`, `/api/agent-tasks/:id`, and `/api/agent-tasks/:id/wait` are available at runtime; then implement the Admin UI agent selector and task agent badges. For further scripted MiMo/Reasonix collaboration, use ``scripts/mimo-bridge-client.mjs``/``.ps1`` where available, run ``recover`` after any interrupted wait, keep using Review Package first, and escalate to focused diff only when risk flags, ``use_worktree=false``, or review notes require it.
