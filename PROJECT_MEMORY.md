...
- Release artifacts were refreshed after P6.16. `npm.cmd run package:installer` produced `artifacts\MiMoBridgeSetup-win10-win11-x64.exe` (55,603,200 bytes) and ignored local `artifacts\MiMoBridge-portable-win10-win11-x64.zip` (55,640,193 bytes). `npm.cmd run validate:release -- -SkipPackageBuild` passed; installer manifest source commit is `fdfbc79`.
...
- P5.4/P6.10/P6.11 safe agent invocation is implemented. `scripts/mimo-bridge-client.mjs` supports MiMo commands `health`, `start`, `wait`, `reply`, `start-and-wait`, `review`, `recover`, plus generic Agent commands `agent-list`, `agent-start`, `agent-wait`, `agent-reply`, `agent-start-and-wait`, `agent-review`, `agent-cancel`, `agent-finish`, `agent-merge`, `agent-discard`, `agent-delete`, and `agent-queue`. Input comes from UTF-8 JSON file or stdin; wait uses MCP SDK with request timeout greater than `timeout_seconds`; CLI output is compact JSON and exits explicitly to avoid stale client processes. `scripts/mimo-bridge-client.ps1` is a thin launcher that locates Node and forwards arguments without constructing JSON.
...
- P6.7 first safe local-open slice is implemented locally: Admin UI task detail has "打开任务文件夹" and Reasonix-only "打开会话文件夹" buttons. Browser requests only `POST /api/tasks/:id/open` with `action=task_folder|session_folder`; it never sends arbitrary paths. The daemon resolves Worktree -> original allowed workspace fallback for task folders, and validates Reasonix session folders under configured `REASONIX_HOME`. Raw local paths are not returned to the browser. Directly opening Reasonix GUI to a specific session is still future work.
...
- Latest package build after P6.11 succeeded with `npm.cmd run package:installer`. Artifacts: `artifacts\MiMoBridgeSetup-win10-win11-x64.exe` (55,599,616 bytes) and ignored local `artifacts\MiMoBridge-portable-win10-win11-x64.zip` (55,636,480 bytes). `npm.cmd run validate:release -- -SkipPackageBuild` passed and wrote ignored local `artifacts\release-validation.json`; installer self-test exit was 0.
