...
- P5.4/P6.10 safe agent invocation is implemented. `scripts/mimo-bridge-client.mjs` supports MiMo commands `health`, `start`, `wait`, `start-and-wait`, `review`, `recover`, plus generic Agent commands `agent-list`, `agent-start`, `agent-wait`, `agent-start-and-wait`, `agent-review`, `agent-cancel`, `agent-finish`, `agent-merge`, `agent-discard`, `agent-delete`, and `agent-queue`. Input comes from UTF-8 JSON file or stdin; wait uses MCP SDK with request timeout greater than `timeout_seconds`; CLI output is compact JSON and exits explicitly to avoid stale client processes. `scripts/mimo-bridge-client.ps1` is a thin launcher that locates Node and forwards arguments without constructing JSON.
